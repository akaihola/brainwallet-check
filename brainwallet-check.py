...
Takes in string as an argument, generates private key and address, then checks
blockchain.info to see if it is in use

Most code taken from https://bitcointalk.org/index.php?topic=84238.0 - thank
you JeromeS!

You need libpcre3-dev for the GET request to work to blockchain.info (apt get
libpcre3-dev)
...
def addy(pk):
    ...
    while pubnum != 0:
        ...
        pubnum /= 58
    address = ''
    for l in ['123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'[x]
              for x in pubnumlist]:
        address = l + address
    return '1' + address
...
if __name__ == "__main__":
    ...
    firstseen = os.popen("GET http://blockchain.info/q/addressfirstseen/"
                         + str(bcaddy)).read()
    amount = os.popen("GET http://blockchain.info/q/addressbalance/"
                      + str(bcaddy)).read()
